...
- 自动更新清单同时包含两种架构，更新时自动选择当前 Mac 对应的版本。

## v0.6.0 · 2026-07-31

Manager Bot 托管、首启引导和定时任务体验升级：

- 新增 Manager Bot 管理流程，可在 Susie 内创建、接入和管理托管 Bot，并自动同步轮换后的 token。
- 首启引导改为 Manager Bot 优先的五步流程，依次完成 Owner 绑定、创建会话 Bot、选择会话和准备 Agent。
- 「频道」更名为「渠道」、「历史」更名为「会话」；渠道显示 Telegram 名称和用户名，会话按渠道分组。
- 定时任务可让 Agent 主动向目标会话发送多条消息和文件，并记录每次投递结果。
- 修复 Codex CLI 检测误认项目内置版本，以及安装其他 Agent 后默认助手仍指向不可用 Codex 的问题。
