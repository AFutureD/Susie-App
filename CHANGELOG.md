...
- 全面更新桌面界面：统一按钮、表单、弹窗、提示、侧边栏与空状态的视觉和交互。
- 优化渠道列表的信息层级与密度：状态点、标题和 username 对齐，标题与副标题更易区分。
- 「新增助手」移到模块标题右侧；删除、卸载与覆盖操作统一使用应用内确认弹窗。
- 会话历史采用新的消息布局与自动滚动体验，向上阅读旧消息时不再被新消息强制拉回底部。
