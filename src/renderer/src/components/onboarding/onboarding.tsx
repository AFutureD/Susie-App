...
// 首启引导：两步（添加频道 → 会话绑定），把「频道页 + 会话绑定面板」的最小路径串起来。
// config 仍是唯一事实源：向导所有写操作走既有 IPC，界面随 config:state 广播重派生。
// 完成/跳过后写 localStorage 标记，之后不再出现。

type WizardStep = 'pending' | 'channel' | 'binding' | 'done' | 'closed'
...
export function OnboardingOverlay() {
  ...
  useEffect(() => {
    ...
    if (next === null) {
      // 配置已成形（如手写 config.toml / 从 Python 版迁移）→ 补标记，避免以后清空绑定时误弹
      if (!done && state.lastError === null) localStorage.setItem(ONBOARDING_DONE_KEY, '1')
      setStep('closed')
      return
    }
    if (next === 'binding') setChannelId(Object.keys(state.config.channels)[0] ?? null)
    setStep(next)
  }, [step, state])

  // 绑定步需要 bot 深链；引导中途退出后恢复到第 2 步时，用已存 token 反查 username
  useEffect(() => {
    if (step !== 'binding' || botUsername !== null || channelId === null || state === null) return
  ...
  const effectiveStep = step === 'binding' && !channelReady ? 'channel' : step
